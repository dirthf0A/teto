...
logger = get_logger("app.services.asset_discovery")
...
# Enhanced discovery modules — imported with fallback for graceful degradation
try:
    from app.services import search_engine_enum as _search_engine_enum
except ImportError:
    _search_engine_enum = None  # type: ignore

try:
    from app.services import github_asset_discovery as _github_discovery
except ImportError:
    _github_discovery = None  # type: ignore

try:
    from app.services import whois_pivoting as _whois_pivoting
except ImportError:
    _whois_pivoting = None  # type: ignore
...
def discover_subdomains(domain: str, environment: str) -> List[AssetCandidate]:
    ...
    def _run_sources(suffix: str) -> None:
        ...
        # Enhanced: search engine enumeration
        if _search_engine_enum is not None:
            try:
                for entry in _search_engine_enum.discover(suffix, limit=100):
                    _add_host(entry.hostname, entry.source)
            except Exception as e:
                logger.debug('optional feature error', error=str(e))
                pass

        # Enhanced: GitHub asset discovery
        if _github_discovery is not None:
            try:
                for entry in _github_discovery.discover_subdomains(suffix, limit=150):
                    _add_host(entry.hostname, entry.source)
            except Exception as e:
                logger.debug('optional feature error', error=str(e))
                pass
    ...
    while pending and depth < max_depth:
        ...
        # Enhanced: WHOIS pivoting for IP ranges from domain registration data
        if _whois_pivoting is not None and depth == 1:
            try:
                pivot_results = _whois_pivoting.pivot_from_domain(root_domain)
                for pivot in pivot_results:
                    for related in pivot.related_domains:
                        _add_host(related, f"whois-{pivot.pivot_type}", allow_external=include_external)
            except Exception as e:
